...
class Config:
    ...
    KAFKA_TOPICS: dict = field(default_factory=lambda: {
        "ohlcv_data": "exchange_ohlcv",
        "trade_data": "exchange_trades",
        "orderbook_updates": "exchange_orderbook",
    ...
    TRADING_FEE_RATE: float = float(os.getenv("TRADING_FEE_RATE", "0.001"))  # Default to 0.1% for LBank
    ...
    MAX_DRAWDOWN_PERCENT: float = float(os.getenv("MAX_DRAWDOWN_PERCENT", "0.20"))
    ...
    CORRELATION_THRESHOLD: float = float(os.getenv("CORRELATION_THRESHOLD", "0.80"))
